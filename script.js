...
if (document.getElementById('cart-items')) {
    ...
    Object.values(groupedCart).forEach(it => {
        ...
        div.innerHTML = `
            ...
            <div style="margin-top:0.7rem;display:flex;align-items:center;gap:0.5rem;">
                <button class="decrease-qty" data-id="${it.id}">-</button>
                <span class="cart-qty" style="min-width:2em;text-align:center;">${it.count}</span>
                <button class="increase-qty" data-id="${it.id}">+</button>
                <button class="remove-item" data-id="${it.id}" style="background:#dc3545;color:#fff;border-radius:5px;padding:0.3rem 0.7rem;">حذف</button>
            </div>
        `;
    ...
    // مدیریت دکمه‌های افزایش/کاهش/حذف
    cartItemsDiv.querySelectorAll('.increase-qty').forEach(btn => {
    ...
    cartItemsDiv.querySelectorAll('.decrease-qty').forEach(btn => {
        btn.addEventListener('click', e => {
            const id = btn.dataset.id;
